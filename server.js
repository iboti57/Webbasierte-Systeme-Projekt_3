...
function signIn(req, res) {
    ...
    query("SELECT * FROM users WHERE name = ? AND password = ?;", [signInName, signInPass]).then(function (result) {
        if (result.length === 0) {
            res.sendStatus(404);
        }
        else {
...
function addTask(req, res) {
    ...
    if (taskName !== undefined && taskDate !== undefined) {
        query("INSERT INTO tasks (name,titel, faelligkeit) VALUES (?,?,?);", [req.session.signInName, taskName, taskDate]).then(function () {
...
function getTasks(req, res) {
    query("SELECT id, titel, faelligkeit FROM tasks WHERE name = ?;", [req.session.signInName]).then(function (result) {
        res.json(result);
    }).catch(function () {
        // DBS-Fehler
        res.sendStatus(500);
